...
Model Deployment Script:
 ...
 - Outputs the global expected value (model baseline) and then the individual feature impacts.
...
def load_artifacts():
    ...
    Assumes "xgboost_house_price_model.joblib", "target_encoder.joblib", and optionally "town_stats.joblib" exist.
    """
...
def fill_town_aggregates(df, town_stats):
    """
    Given a DataFrame and a dictionary of town aggregated statistics (town_stats),
    fill in extended town features: 'town_mean', 'town_median', 'town_std', 'town_count'.
    """
...
def prepare_features(df, town_stats):
    ...
    Expected features (as used in training):
...
def predict_house_price(input_data):
    ...
    # Load town aggregated statistics.
    _, _, town_stats = load_artifacts()

    # Prepare the final features (including extended town features).
    X_input = prepare_features(processed_input, town_stats)

    # Work on an explicit copy to avoid warnings.
    ...
    # Apply the target encoder to the categorical columns (ensure same shape as during training).
    X_input.loc[:, categorical_cols] = encoder.transform(X_input[categorical_cols]).astype(float)

    # Cast all features to numeric.
...
def compute_shap_explanation(model, X_input):
    ...
    Also returns the model's global expected value (baseline).
    """
    ...
    shap_df = pd.DataFrame({
        ...
        'SHAP Value': shap_values[0]  # for the first (or only) instance
    })
...
if __name__ == "__main__":
    ...
    sample_input = {
        ...
        "storey_range": "04 to 06",  # Original floor range string
        "town": "YISHUN",  # Town information
        "flat_type": "4-room",
        "flat_model": "Simplified",
        "street_name": "ANG MO KIO AVE 10",  # Fine-grained location feature
        "floor_area_sqm": 84,
    ...
    # Load the model and compute SHAP values for the prediction.
    model, _, _ = load_artifacts()
    global_expected_value, shap_df = compute_shap_explanation(model, features_used)
    print("\nGlobal Expected Value (Baseline):", global_expected_value)
    print("\nInput Variable Importance (SHAP values) for the prediction:")
    print(shap_df.to_string(index=False))

    # Optional: Generate and save an interactive force plot (requires appropriate front-end or Jupyter).
    # Uncomment the lines below to create the force plot.
    # shap.initjs()
    # force_plot = shap.force_plot(explainer.expected_value, shap_df['SHAP Value'].values, features_used.iloc[0, :])
